...
class SimpleHTTPServer:
    ...
    def send_http_respond(self, client_socket: socket.socket, msgbody: typing.Union[bytearray, bytes], additional_headers: dict, status_code: str="200 OK"):
        ...
        additional_headers.update({key.strip().title(): self.additional_headers[key].strip() 
                                        for key in self.additional_headers if key not in additional_headers})
    ...
    def generate_access_key():
        ...
        acckey = base64.b64encode(k).decode("utf-8").replace("=", "").replace("+", "")
        return acckey
            ...
            if len(req) == 3:
                ...
                if request_file_rel_path in self.files:
                    ...
                    if inspect.isfunction(f['file']):
                        ...
                        if "X-Will-Quit-After" in resp.headers and resp.headers["X-Will-Quit-After"] == "YES":
                            self.stop()
                        
                    elif os.path.isfile(f['file']):

                        ff = io.open(f['file'], "rb")
                        t = ff.read()
